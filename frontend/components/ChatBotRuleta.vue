<template>
  <div class="bg-white p-4 rounded-lg shadow-md h-full flex flex-col">
    ...
    <!-- Números para copiar fácilmente -->
    <div class="mb-4 bg-blue-50 p-4 rounded-lg border border-blue-200">
      <div class="flex justify-between items-center mb-3">
        <h3 class="text-lg font-semibold text-blue-700">Números para Copiar</h3>
        <button
          @click="refreshCSVPredictions"
          class="bg-blue-600 text-white px-2 py-1 rounded text-xs"
      ...
      <div v-if="csvNumbers" class="bg-white p-3 rounded shadow-sm">
        <div class="flex justify-between items-center mb-2">
          <span class="font-medium text-gray-700">Números separados por comas</span>
          <button
            @click="copyToClipboard(csvNumbers)"
...
const apiBaseUrl = ref(process.server ? '' : window.location.origin.includes('localhost') ? 'http://localhost:8080' : window.location.origin);
...
const lastKnownNumber = ref<number | null>(null);
...
// Variables para números CSV
const csvNumbers = ref('');
...
// Función para verificar victoria/derrota de un número detectado automáticamente
const verifyWinLossForNumber = async (number: number) => {
  console.log(`🔍 Verificando victoria/derrota para número detectado automáticamente: ${number}`);

  // Crear estructura de resultado similar a la existente
  const result = {
    isWinning: false,
  ...
  // ========== PASO 1: VERIFICAR GRUPOS PRINCIPALES (VICTORIA/DERROTA) ==========
  const isInGroup20 = numberGroups.value.group20?.includes(number) || false;
  const isInGroup12 = numberGroups.value.group12?.includes(number) || false;

  if (isInGroup20 || isInGroup12) {
    result.isWinning = true;

    // Identificar grupos principales
    if (isInGroup20) {
      result.matchingGroups.push('Grupo 20');
      result.predictedNumbers = [...(numberGroups.value.group20 || [])];
    }
    if (isInGroup12) {
      result.matchingGroups.push('Grupo 12');
      if (!result.predictedNumbers.length) {
        result.predictedNumbers = [...(numberGroups.value.group12 || [])];
      }
    }

    // Incrementar contador de victorias
    winCount.value++;
    console.log(`✅ VICTORIA! Nuevo conteo: ${winCount.value}`);
  } else {
    // Incrementar contador de derrotas
    loseCount.value++;
    console.log(`❌ DERROTA! Nuevo conteo: ${loseCount.value}`);
  }

  // ========== PASO 2: VERIFICAR GRUPOS ADICIONALES PARA ESTADÍSTICAS ==========

  // Verificar otros grupos estándar
  Object.entries(numberGroups.value).forEach(([groupName, numbers]) => {
    if (groupName !== 'group20' && groupName !== 'group12' && numbers && numbers.includes(number)) {
      result.matchingGroups.push(getGroupLabel(groupName));
      // Incrementar contador específico de grupo
  ...
  Object.entries(localStatGroups.value).forEach(([groupName, numbers]) => {
    if (numbers && numbers.includes(number)) {
      result.matchingGroups.push(getLocalStatGroupLabel(groupName));
      if (groupWinStats.value[groupName] !== undefined) {
        groupWinStats.value[groupName]++;
      }
    }
  ...
  Object.entries(aiGroups.value).forEach(([groupName, numbers]) => {
    if (numbers && numbers.includes(number)) {
      result.matchingGroups.push(getAIGroupLabel(groupName));
      if (groupWinStats.value[groupName] !== undefined) {
        groupWinStats.value[groupName]++;
      }
    }
  ...
  // ========== PASO 3: VERIFICAR SECTORES DE RULETA ==========
  try {
    const rouletteSectors = getRouletteSectors();
    const foundSectors = [];

    if (rouletteSectors.voisinsDeZero.includes(number)) {
      foundSectors.push('Vecinos del Cero');
      result.rouletteSector = 'Vecinos del Cero';
    }
    if (rouletteSectors.tiers.includes(number)) {
      foundSectors.push('Tercios del Cilindro');
      if (!result.rouletteSector) result.rouletteSector = 'Tercios del Cilindro';
    }
    if (rouletteSectors.orphelinsPlein.includes(number)) {
      foundSectors.push('Huérfanos');
      if (!result.rouletteSector) result.rouletteSector = 'Huérfanos';
    }
    if (rouletteSectors.jeuZero.includes(number)) {
      foundSectors.push('Juego del Cero');
      if (!result.rouletteSector) result.rouletteSector = 'Juego del Cero';
    }

    // Agregar sectores a grupos encontrados
    foundSectors.forEach(sector => {
      if (!result.matchingGroups.includes(sector)) {
        result.matchingGroups.push(sector);
      }
    });
  ...
  console.log(`📊 RESULTADO para número ${number}: ${result.isWinning ? 'VICTORIA' : 'DERROTA'}`);
  console.log(`📝 Grupos encontrados: ${result.matchingGroups.join(', ')}`);
...
onMounted(async () => {
  ...
  // Cargar predicciones CSV iniciales con un pequeño retraso para asegurar que el backend esté listo
  setTimeout(async () => {
  ...
  // Inicializar auto-actualización en tiempo real
...
onBeforeUnmount(() => {
  ...
  // Limpiar intervalo de auto-actualización
...
// Función para inicializar auto-actualización en tiempo real
const startAutoRefresh = async () => {
  // Obtener el número inicial para comparar
  const numbers = await getLastRouletteNumbers(1);
  if (numbers && numbers.length > 0) {
    lastKnownNumber.value = numbers[0].number;
  }

  // Configurar intervalo de verificación cada 3 segundos
  autoRefreshInterval = setInterval(async () => {
    try {
      const latestNumbers = await getLastRouletteNumbers(1);
      if (latestNumbers && latestNumbers.length > 0) {
        const latestNumber = latestNumbers[0].number;

        // Si hay un nuevo número, actualizar el chat
        if (lastKnownNumber.value !== null && lastKnownNumber.value !== latestNumber) {
          console.log(`🔔 Nuevo número detectado: ${latestNumber} (anterior: ${lastKnownNumber.value})`);

          // Actualizar número conocido
          lastKnownNumber.value = latestNumber;

          // Primero verificar victoria/derrota con los grupos ACTUALES (antes de regenerar)
          const currentResult = await verifyWinLossForNumber(latestNumber);

          // Agregar mensaje automático al chat con información de victoria/derrota
          const resultMessage = currentResult.isWinning
  ...
  console.log('✅ Auto-actualización en tiempo real iniciada');
};
...
const refreshAllGroups = async () => {
  try {
    ...
    // Actualizar predicciones CSV después de regenerar grupos
    console.log('🔄 Actualizando predicciones CSV después de regenerar grupos...');
    await refreshCSVPredictions();
...
const sendMessage = async () => {
  ...
  if (numberToAdd !== null) {
    ...
    try {
      // PASO 0: Verificar si el número ya existe en Redis (prevención de duplicados)
      const isDuplicate = await checkIfNumberExists(numberToAdd);
      
      if (isDuplicate) {
        console.log(`⏭️ DUPLICADO DETECTADO: número ${numberToAdd} reconocido por voz - ya existe en Redis`);
        
        // IMPORTANTE: Aún calcular si fue ganador o perdedor aunque sea duplicado
        const currentResult = checkGroupsForNumber(numberToAdd);
        
        let duplicateMessage = `🎤 Reconocí "${numberToAdd}" por voz, pero este número ya existe en el historial de Redis.\n\n⚠️ Para evitar duplicados con el scrapper automático, no se procesará nuevamente.\n\n💡 El scrapper ya capturó este número automáticamente.`;
        
        // Añadir información de victoria/derrota
        if (currentResult.isWinning) {
          duplicateMessage += `\n\n✅ RESULTADO: ¡Este número habría sido una VICTORIA! Se encontró en nuestras predicciones.`;
        } else {
        ...
        // Emitir evento de resultado aunque sea duplicado para actualizar contadores
        if (emitter) {
        ...
        return; // Salir sin procesar en base de datos
      }
  ...
  if (containsMultipleNumbers || message.includes(',')) {
    console.log(`🔢 Procesando múltiples números individualmente: ${message}`);
      
    // NUEVO: Procesar cada número individualmente para generar grupos y verificar victorias
    try {
      await processIndividualNumbersSequentially(message);
      return;
    } catch (error) {
  ...
  // FALLBACK: Si no hay comas, procesamiento original
  if (false) { // Desactivado temporalmente, usar solo procesamiento individual
    try {
  ...
  // Verificar si el mensaje contiene un único número
    ...
    try {
      ...
      // PASO 0: Verificar si el número ya existe en Redis (prevención de duplicados)
      const isDuplicate = await checkIfNumberExists(number);
      
      if (isDuplicate) {
        console.log(`⏭️ DUPLICADO DETECTADO: número ${number} - ya existe en Redis`);
        
        // IMPORTANTE: Aún calcular si fue ganador o perdedor aunque sea duplicado
        const currentResult = checkGroupsForNumber(number);
        
        let duplicateMessage = `⚠️ El número ${number} ya existe en el historial de Redis. Para evitar duplicados con el scrapper automático, no se procesará nuevamente.\n\n💡 Tip: El scrapper ya está capturando este número automáticamente.`;
      ...
      if (processed && processed.processedCount > 0) {
        ...
        // Procesar resultados de predicciones del backend enhanced
        if (processed.predictionResults && processed.predictionResults.length > 0) {
          // Usar resultados del backend enhanced
          processEnhancedPredictionResults(processed.predictionResults, number);

          // Emitir evento de resultado de la apuesta
          if (emitter) {
            const hasWinning = processed.predictionResults.some((result: any) => result.overall_winner);
            emitter.emit('number-played', {
              number,
          ...
          // Fallback al sistema local si no hay resultados del backend
          let resultMessage = "";
          let winResult = false;
...
// Función para procesar y mostrar resultados de predicciones del backend enhanced
const processEnhancedPredictionResults = (predictionResults: any[], actualNumber: number) => {
  ...
  predictionResults.forEach((result, index) => {
    setTimeout(() => {
      const isWinning = result.overall_winner;
      const winningGroups = result.group_results
        .filter((gr: any) => gr.is_winner)
        .map((gr: any) => `${gr.group_name} (${gr.group_size} números)`);

      let message = '';
      if (isWinning) {
        message = `🎉 ¡PREDICCIÓN ACERTADA! El número ${actualNumber} estaba en ${winningGroups.length} de nuestros grupos:\n`;
        message += winningGroups.map(group => `✅ ${group}`).join('\n');
        message += `\n📊 Tasa de éxito: ${result.analysis.success_rate}`;
        message += `\n🤖 Modelo usado: ${result.model_used}`;
      } else {
        message = `❌ El número ${actualNumber} no estaba en ninguno de nuestros grupos de predicción.`;
        message += `\n📊 ${result.analysis.total_groups} grupos analizados, 0 aciertos`;
        message += `\n🤖 Modelo usado: ${result.model_used}`;
      }
...
// NUEVA FUNCIÓN: Verificar si un número ya existe en Redis para evitar duplicados
const checkIfNumberExists = async (number: number): Promise<boolean> => {
  try {
    console.log(`🔍 Verificando si el número ${number} ya existe en Redis...`);
    
    // Obtener los últimos números de Redis
    const recentNumbers = await getLastRouletteNumbers(50); // Verificar últimos 50 números
    
    if (!recentNumbers || recentNumbers.length === 0) {
      console.log(`📝 No hay números en Redis, ${number} es nuevo`);
      return false;
    }
    
    // Verificar si el número ya existe en los últimos números
    const exists = recentNumbers.some(item => item.number === number);
    
    if (exists) {
      console.log(`⚠️ DUPLICADO DETECTADO: El número ${number} ya existe en Redis`);
      console.log(`📊 Últimos números en Redis: [${recentNumbers.slice(0, 10).map(item => item.number).join(', ')}...]`);
      return true;
    } else {
      console.log(`✅ Número ${number} es nuevo, no existe en Redis`);
      return false;
    }
  } catch (error) {
    console.error('Error verificando duplicados en Redis:', error);
    // En caso de error, permitir el procesamiento para no bloquear el flujo
    return false;
  }
};

// NUEVA FUNCIÓN: Procesar cada número individualmente para generar grupos y verificar victorias
const processIndividualNumbersSequentially = async (numbersString: string) => {
  ...
  console.log(`📊 Números válidos extraídos: [${numbersArray.join(', ')}]`);
  console.log(`⏰ Orden temporal: ${numbersArray[numbersArray.length - 1]} (más viejo) → ${numbersArray[0]} (más reciente)`);
  
  // IMPORTANTE: Procesar de derecha a izquierda (más viejo al más reciente)
  // Ejemplo: "4,16,3,4,14,22,20,20" → procesar en orden: 20,20,22,14,4,3,16,4
  const processingOrder = [...numbersArray].reverse();
  console.log(`🔄 Orden de procesamiento: [${processingOrder.join(', ')}] (viejo → reciente)`);
  ...
  // Procesar cada número individualmente en orden cronológico correcto
  for (let i = 0; i < processingOrder.length; i++) {
    const number = processingOrder[i];
    const originalPosition = numbersArray.length - i; // Posición original para mostrar al usuario
    console.log(`\n======= PROCESANDO NÚMERO ${i + 1}/${processingOrder.length}: ${number} (Posición original: ${originalPosition}) =======`);
    ...
    try {
      ...
      console.log(`🔍 Procesando número ${number} (no es duplicado)...`);
      ...
      if (processed && processed.processedCount > 0) {
        ...
        if (currentResult.isWinning) {
          victories++;
          resultMessage = `✅ Número ${number} (pos. ${originalPosition}): ¡VICTORIA! ${getWinningMessage(number, currentResult)}`;
        } else {
          defeats++;
          resultMessage = `❌ Número ${number} (pos. ${originalPosition}): DERROTA. No estaba en las predicciones activas.`;
        }
        ...
        // Pausa pequeña entre números para mejor UX y permitir que se actualicen los datos
  ...
  console.log(`Duplicados saltados: ${duplicatesSkipped}`);
  ...
  if (totalProcessed > 0 || duplicatesSkipped > 0) {
    const efficiencyPercentage = totalProcessed > 0 ? Math.round((victories / totalProcessed) * 100) : 0;
    let summaryMessage = `📊 RESUMEN: Se procesaron ${totalProcessed} números nuevos`;
    
    if (duplicatesSkipped > 0) {
      summaryMessage += ` (${duplicatesSkipped} duplicados saltados)`;
    }
    ...
    // Generar nuevas predicciones para el siguiente número solo si se procesaron números nuevos
    if (totalProcessed > 0) {
      setTimeout(async () => {
        try {
          const statNumbers = await generateStatPredictions(6);
  ...
  // Emitir evento final solo si se procesaron números nuevos
  if (emitter && totalProcessed > 0) {
    emitter.emit('numbers-updated');
    if (numbersArray.length > 0) {
      // El último número procesado es el más reciente (primera posición en la entrada original)
      emitter.emit('last-number-played', numbersArray[0]);
    }
  }
};
...
const handleRecognizedNumber = async (numberToAdd: number) => {
  ...
  try {
    ...
    if (emitter) {
      ...
      // Emitir evento para activar predicciones AI
      emitter.emit('number-added', numberToAdd);
    }
...
// Funciones para CSV - Usar los mismos grupos que se muestran en "Grupos de números"
const refreshCSVPredictions = async () => {
  try {
    console.log('🔄 Generando predicciones CSV desde grupos locales del frontend...');
    
    // Usar los grupos que ya están generados en el frontend para mantener consistencia
    const currentGroups = numberGroups.value;
    
    if (!currentGroups || Object.keys(currentGroups).length === 0) {
      console.log('⚠️ No hay grupos generados, generando nuevos grupos...');
      await refreshAllGroups();
    }
    
    // Crear formato CSV usando los mismos grupos que se muestran en la UI
    const csvParts: string[] = [];
    
    // Agregar grupos estándar en el mismo orden que se muestran
    if (currentGroups.group20 && currentGroups.group20.length > 0) {
      const sorted = [...currentGroups.group20].sort((a, b) => a - b);
      csvParts.push(sorted.join(','));
      console.log('📊 Grupo 20:', sorted.join(','));
    }
    
    if (currentGroups.group15 && currentGroups.group15.length > 0) {
      const sorted = [...currentGroups.group15].sort((a, b) => a - b);
      csvParts.push(sorted.join(','));
      console.log('📊 Grupo 15:', sorted.join(','));
    }
    
    if (currentGroups.group12 && currentGroups.group12.length > 0) {
      const sorted = [...currentGroups.group12].sort((a, b) => a - b);
      csvParts.push(sorted.join(','));
      console.log('📊 Grupo 12:', sorted.join(','));
    }
    
    if (currentGroups.group8 && currentGroups.group8.length > 0) {
      const sorted = [...currentGroups.group8].sort((a, b) => a - b);
      csvParts.push(sorted.join(','));
      console.log('📊 Grupo 8:', sorted.join(','));
    }
    
    if (currentGroups.group6 && currentGroups.group6.length > 0) {
      const sorted = [...currentGroups.group6].sort((a, b) => a - b);
      csvParts.push(sorted.join(','));
      console.log('📊 Grupo 6:', sorted.join(','));
    }
    
    if (currentGroups.group4 && currentGroups.group4.length > 0) {
      const sorted = [...currentGroups.group4].sort((a, b) => a - b);
      csvParts.push(sorted.join(','));
      console.log('📊 Grupo 4:', sorted.join(','));
    }
    
    // Crear el formato final separado por " | "
    if (csvParts.length > 0) {
      csvNumbers.value = csvParts.join(' | ');
      console.log('✅ Números CSV generados desde grupos locales:', csvNumbers.value.substring(0, 100) + '...');
      console.log('🔍 Verificación: Los números CSV ahora coinciden exactamente con los mostrados en "Grupos de números"');
    } else {
      console.log('⚠️ No se pudieron generar grupos válidos');
      csvNumbers.value = '';
    }
    
  } catch (error) {
    console.error('❌ Error generando predicciones CSV desde grupos locales:', error);
    csvNumbers.value = '';
  }
};
